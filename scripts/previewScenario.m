...
function previewScenario(Agent, Task, scenario_id)
    ...
    axis_limit = max(max([Task.Te]),max(([Agent.Ft])));
    word_length = 0.2 * axis_limit;
    ...
    for a = 1:A
        ...
        flight_time = barh(a, [Agent(a).Ft], barWidth,'stacked', 'LineWidth', 1.5, 'FaceColor', color);
        ...
        if Agent(a).Ft_0 ~= 0
            initial_flight_time(1).FaceColor = [0.2 0.2 0.2];
            initial_flight_time(1).FaceAlpha = 0.5;
        end
    end
    ...
    for t = 1:T
        execution_time = barh(t - 1, [Task(t).Te], barWidth,'stacked', 'LineWidth', 1.5);
    ...
    set(gca,'FontSize', 12, 'XGrid', 'on', 'YGrid', 'off','xminorgrid','on');
